  ...
  render(){
    ...
    return (
      <div style = { {marginLeft: px } } className={`accordion-item ${checked ? 'active':""}`} id = { this.props.items.id } onClick = { this.send.bind(this) }>
        <div className="accordion-title">我是:{this.props.items.name} checked: { checked ? "被选了":"未被选" }<Link to="/Doing">去做题</Link></div>
        <div className={`${checked ? 'in':""} ${"accordion-body collapse"}`}>{subsarry}</div>
      </div>
    )
  }
}